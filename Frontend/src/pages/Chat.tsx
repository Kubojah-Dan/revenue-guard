  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      text: "Hello! I'm the Revenue Process Twin Narrator powered by Ollama. Ask me about any customer, revenue leakage pattern, or recovery opportunity.",
    },
  ]);
  ...
  async function handleSend(query?: string) {
    ...
    try {
      ...
      setMessages(prev =>
        prev.map(m =>
          m.id === assistantId
            ? { ...m, text: `⚠ Ollama unavailable: ${err instanceof Error ? err.message : "Unknown error"}. Make sure Ollama is running on ${import.meta.env.VITE_OLLAMA_URL ?? "http://localhost:11434"} with the ${import.meta.env.VITE_OLLAMA_MODEL ?? "llama3"} model loaded.` }
            : m
        )
      );
    } finally {
